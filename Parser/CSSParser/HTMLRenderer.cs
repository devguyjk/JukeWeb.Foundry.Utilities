		...
		static string Render(SimpleSelector ss) {
			...
			if (ss.Combinator.HasValue) {
				switch (ss.Combinator.Value) {
					case JukeWeb.Foundry.Utilities.Parser.CSSParser.Combinator.PrecededImmediatelyBy: txt.Append(" <span class=\"combinator\">+</span> "); break;
					case JukeWeb.Foundry.Utilities.Parser.CSSParser.Combinator.ChildOf: txt.Append(" <span class=\"combinator\">&gt;</span> "); break;
					case JukeWeb.Foundry.Utilities.Parser.CSSParser.Combinator.PrecededBy: txt.Append(" <span class=\"combinator\">~</span> "); break;
				}
			}
		...
		static string Render(Term t) {
			...
			if (t.Type == TermType.Function) {
				...
				if (t.Unit.HasValue) {
					...
					if (t.Unit.Value == JukeWeb.Foundry.Utilities.Parser.CSSParser.Unit.Percent) {
						txt.Append("%");
					} else {
