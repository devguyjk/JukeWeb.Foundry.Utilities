...
.
			*/
			...
			if (type == TermType.Function) {
				...
				txt.Append(val);
				if (unit.HasValue) {
					if (unit.Value == JukeWeb.Foundry.Utilities.Parser.CSSParser.Unit.Percent) {
						txt.Append("%");
					} else {
		...
		private int GetRGBValue(Term t) {
			try {
				if (t.Unit.HasValue && t.Unit.Value == JukeWeb.Foundry.Utilities.Parser.CSSParser.Unit.Percent) {
